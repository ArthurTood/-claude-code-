...
describe("permission bubble stack layout", () => {
  ...
  it("prefers a top-centered stack on macOS when bubbleFollowPet is enabled", () => {
    const bounds = layout({
      followPet: true,
      preferTopDock: true,
      bubbleHeights: [180, 180, 180],
      workArea: FHD,
      hitRect: { left: 800, top: 900, right: 920, bottom: 1000 },
    });

    assert.deepStrictEqual(bounds, [
      { x: 790, y: 8, width: 340, height: 180 },
      { x: 790, y: 194, width: 340, height: 180 },
      { x: 790, y: 380, width: 340, height: 180 },
    ]);
  });
  ...
  it("falls back to the legacy pet-side layout when the top stack would overflow", () => {
    const bounds = layout({
      followPet: true,
      preferTopDock: true,
      bubbleHeights: Array(6).fill(200),
      workArea: FHD,
      hitRect: { left: 800, top: 900, right: 920, bottom: 1000 },
    });
    ...
    for (const b of bounds) assert.strictEqual(b.x, 920);
    assert.strictEqual(bounds[0].y, 8);
    assert.strictEqual(bounds[5].y, 1038);
    for (let i = 0; i < bounds.length - 1; i++) {
      assert.ok(bounds[i].y < bounds[i + 1].y);
    }
  });
